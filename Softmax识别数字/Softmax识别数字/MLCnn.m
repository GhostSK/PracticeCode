...
+(double *)relu:(double * )x size:(int)size{
    double *zero = [MLCnn fillVector:0.0 size:size ];
...
+(double)max_pool:(double *)input dim:(int)dim row:(int)row col:(int)col stride:(NSArray *)stride{
    double maxV = input[dim * [stride[0] integerValue] + row * 2 * [stride[1] integerValue] + col * 2];
...
+(double *)bias_init:(int)size{
    return [MLCnn fillVector:0.1 size:size];
}
...
-(double *)filterImage:(double *)image state:(BOOL)isTraining{
    ...
    for (int i = 0; i < _numOfFilter; i++) {
        double *conv = [MLCnn fillVector:0.0 size:row * col * [_filters[i][2] intValue]];
        for (int k = 0; k < [_filters[i][2] integerValue]; k++) {
            ...
            vDSP_vsaddD((conv + k * col * row), 1, &_bias[i][k], (conv + k * row * col), 1, row * col);
            if (inner != NULL) {
                free(inner);
                inner = NULL;
            }
...
-(void)backPropagetion:(double *)loss{
    int row = _outRow;
    int col = _outCol;
    //dropout
    ...
    for (int i = _numOfFilter - 1; i >= 0; i--) {
        ...
        if (i > 0) { //if not the first layer calculate back loss
            ...
            convBackLoss = [MLCnn fillVector:0.0 size:(row +strideRow * 2) * (col + strideCol * 2) * preDim];
            double *curLoss = [MLCnn fillVector:0.0 size:(row + strideRow * 2) * (col + strideCol * 2) * [_filters[i][2] intValue]];
            for (int k = 0; k < [_filters[i][2] intValue]; k++) {
                for (int p = 0; p < row; p++) {
                    for (int q = 0; q < col; q++) {
            ...
            for (int k = 0; k < preDim; k++) {
                ...
                for (int m = 0; m < [_filters[i][2] intValue]; m++) {
                    double *reverseWeight = [MLCnn fillVector:0.0 size:[_filters[i][0] intValue] * [_filters[i][1] intValue]];
        ...
        for (int k = 0; k < [_filters[i][2] intValue]; k++) {
            ...
            for ( int m = 0; m < preDim; m++) {
                ...
                double *weightLoss = malloc(sizeof(double) * [_filters[i][0] intValue]);
                int P = row / 2;
                int Q = col / 2;
