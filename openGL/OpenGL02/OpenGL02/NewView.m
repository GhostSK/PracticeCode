...
-(void)render{
    ...
    if (linkSuccess == GL_FALSE) {  //链接错误
        ...
        NSLog(@"Link Success");
        glUseProgram(self.MyProgram); //成功就使用，避免由于没成功带来的bug
    }
    
    //设置纹理矩阵
    //前三个是顶点坐标，后两个是纹理坐标 中间三个法线坐标被我吃了
    GLfloat attrArr[] ={
    ...
    //加载数据
    [self setupTexture:@"443"];
    
    //或缺shader里的变量，这里记得要在glLinkProgram后面
    GLuint rotate = glGetUniformLocation(self.MyProgram, "rotateMatrix");
    
    float radians = 10 * 3.141592654 / 180;
    float s = sin(radians);
    float c = cos(radians);
...
-(void)setupLayer{
    ...
    //CALayer 默认是透明的 必须将他设为不透明才能让其可见
    self.myEagLayer.opaque = YES;
    //设置庙会属性，在这里设置不维持渲染内容 以及颜色格式为RGBA8
...
-(void)setupContext{
    //指定OpenGL 渲染 API 的版本 在这里我们使用OpenGL ES 2.0
...
-(void)setupFrameBuffer{
    ...
    //将 ——colorRenderBuffer 装配到GL_COLOR_ATTACHMENT0这个装配点上
...
-(GLuint)setupTexture:(NSString *)fileName{
    ...
    //4、绑定纹理到默认的纹理ID （这里只有一张图片孤儿相当于默认于片元着色器里面的colorMap）
    //如果有多张图片不可以这样
